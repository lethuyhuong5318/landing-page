...
function lessonNav(index) {
  ...
  <header class="cluster-lesson-header">
    ...
    <h1>${lesson.title}</h1><p>${lesson.description}</p><div class="cluster-lesson-side" aria-label="Điểm học tập"></div>
  </header>
  <nav class="cluster-lesson-nav" aria-label="Dieu huong bai hoc">
    ...
    <a class="cluster-all" href="/lay-goc-hoa/">Xem 11 b&#224;i h&#7885;c</a>
...
<script type="application/ld+json">${JSON.stringify({
  ...
  return html
    ...
    .replace('</body>', `<script src="/assets/js/chemistry-lesson-progress.js"></script><script>document.addEventListener("DOMContentLoaded",()=>{selectTab("${lesson.id}");document.querySelectorAll(".panel").forEach(p=>{if(p.id!=="${lesson.id}")p.setAttribute("hidden","")});const side=document.querySelector(".cluster-lesson-side"),mascot=document.querySelector(".hero .mascot-badge"),xp=document.querySelector(".hero .xp-badge");if(side){if(mascot)side.appendChild(mascot);if(xp)side.appendChild(xp);}});</script></body>`);
}
...
.cluster-lesson-header{margin-bottom:14px;padding:20px 22px;display:grid;grid-template-columns:minmax(0,1fr) auto;column-gap:24px;overflow:hidden;position:relative;border:1px solid #4aa6e5;border-radius:22px;background:linear-gradient(120deg,#082f55 0%,#0d5b96 58%,#168be0 100%);color:#fff;box-shadow:0 14px 32px rgba(8,47,85,.18)}.cluster-lesson-header:after{content:"";position:absolute;right:-70px;top:-100px;width:230px;height:230px;border-radius:50%;background:rgba(255,209,102,.15)}.cluster-lesson-header>*:not(.cluster-lesson-side){grid-column:1;position:relative;z-index:1}.cluster-home-link{display:inline-flex;min-height:44px;align-items:center;color:#fff;text-decoration:none;font-weight:700}.cluster-kicker{display:block;margin-top:6px;color:#ffd46d;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.cluster-lesson-header h1{margin:8px 0 6px;font-size:clamp(24px,4vw,38px);line-height:1.18}.cluster-lesson-header p{max-width:760px;margin:0;color:#e8f5ff;line-height:1.65}.cluster-lesson-side{grid-column:2;grid-row:1/5;align-self:center;display:flex;align-items:center;gap:14px;position:relative;z-index:2}.cluster-lesson-side .mascot-badge{position:static;flex:0 0 72px;width:72px;height:72px;margin:0;border:3px solid #ffd166;border-radius:50%;background:#fff;box-shadow:0 8px 20px rgba(5,35,62,.25);overflow:hidden}.cluster-lesson-side .mascot-badge img{width:100%;height:100%;object-fit:cover}.cluster-lesson-side .xp-badge{position:static;min-width:104px;min-height:88px;padding:10px 13px;display:grid;place-items:center;border:2px solid #ffd166;border-radius:16px;background:linear-gradient(160deg,#fff7d7,#ffd166);color:#0b365e;box-shadow:0 8px 20px rgba(5,35,62,.2)}.cluster-lesson-side .xp-num{color:#d85b00;font-size:26px;font-weight:900;line-height:1}.cluster-lesson-side .xp-lbl{color:#244f73;font-size:9px;font-weight:800;text-transform:uppercase}.cluster-lesson-side .lvl-lbl{padding:3px 9px;border-radius:999px;background:#14966f;color:#fff;font-size:9px;font-weight:900}.cluster-original-title{font-size:clamp(25px,4vw,40px);font-weight:800;line-height:1.1;margin:0}
.cluster-lesson-nav{display:grid;grid-template-columns:1fr auto 1fr;gap:8px;margin:14px 0}.cluster-lesson-nav a{min-height:54px;padding:8px 12px;display:flex;align-items:center;justify-content:center;gap:9px;border:1px solid #cfe1ed;border-radius:14px;background:#fff;color:#123f67;font-size:11px;font-weight:800;text-align:center;text-decoration:none}.cluster-next{background:#eaf6ff!important}.cluster-nav-arrow{display:grid;flex:0 0 34px;width:34px;height:34px;place-items:center;border-radius:999px;background:#ffc83d;color:#0c3a62;font-size:22px;line-height:1;box-shadow:0 4px 10px rgba(216,155,0,.25)}.cluster-lesson-picker{grid-column:1/-1;display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:10px;padding:10px 12px;border:1px solid #cfe1ed;border-radius:14px;background:#f7fbfe;color:#123f67;font-size:11px;font-weight:800}.cluster-lesson-picker select{width:100%;min-height:44px;padding:0 38px 0 12px;border:1px solid #b9d6e7;border-radius:11px;background:#fff;color:#123f67;font:700 12px inherit}
...
@media(max-width:680px){.cluster-lesson-nav{grid-template-columns:1fr 1fr}.cluster-all{grid-column:1/-1;grid-row:2}.cluster-prev,.cluster-next{flex-direction:column;line-height:1.35}.cluster-prev>span:last-child,.cluster-next>span:first-child{display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:2}.cluster-lesson-picker{grid-template-columns:1fr}.cluster-related>div{grid-template-columns:1fr}.cluster-lesson-header{padding:16px;grid-template-columns:1fr}.cluster-lesson-side{grid-column:1;grid-row:auto;margin-top:14px;justify-content:space-between;border-top:1px solid rgba(255,255,255,.2);padding-top:14px}.cluster-lesson-side .mascot-badge{width:58px;height:58px;flex-basis:58px}.cluster-lesson-side .xp-badge{min-height:68px;min-width:124px;grid-template-columns:auto 1fr;column-gap:7px}.cluster-lesson-side .xp-num{grid-row:1/3;font-size:24px}.cluster-breadcrumb{margin-top:8px}}
`;
